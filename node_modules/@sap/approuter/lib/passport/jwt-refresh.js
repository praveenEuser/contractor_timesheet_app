...
function doRefresh(app, session) {
  ...
  // Check if session still exists in memory store before attempting refresh
  const memoryStore = app.get('memoryStore');
  memoryStore.get(session.id, function (err, currentSession) {
    if (err || !currentSession) {
      tracer.info('Session %s no longer exists, skipping JWT refresh', session.id);
      return;
    }

    if (!currentSession.user || !currentSession.user.token) {
      tracer.info('JWT could not be refreshed, session user or session user token is not defined');
      return;
    }
    let oauthOptions = currentSession.user.token.oauthOptions;
    let requestOptions = {
      url: oauthOptions.tokenURL,
      form: {
        'grant_type': 'refresh_token',
        'refresh_token': currentSession.user.token.refreshToken,
    ...
    requestOptions.timeout = currentSession.user.token.expiryDate - Date.now();
    let checkTimeout = function () {
      if (currentSession.user.token.expiryDate - Date.now() < 0) {
        tracer.info('JWT for session %s could not be refreshed!', currentSession.id);
    ...
    passportUtils.callUaa(requestOptions, checkTimeout, function (err, uaaResponse) {
      if (err) {
        tracer.warning(err, 'Session %s', currentSession.id);
        return logger.error('Unable to refresh the JWT for session %s , number of attempts done: %d, error: %s', currentSession.id, passportUtils.MAX_ATTEMPTS_NUMBER, err.message);
      }
      ...
      let updatedSession;
      app.get('memoryStore').update(currentSession.id, function updateSession(newSession) {
        if (!newSession) {
          return;
        }
        let options = {
          ...
          oauthOptions: currentSession.user.token.oauthOptions
        };
