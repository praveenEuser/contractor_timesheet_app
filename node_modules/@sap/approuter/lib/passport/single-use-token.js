...
async function createSessionByAuthCode(req, cb) {
  try {
    ...
    const oidcToken = await exports.fetchServiceToken(headerUtils.getCorrelationId(req),req.query && req.query.approuterAuthCode);
...
async function fetchServiceToken(correlationId, approuterAuthCode) {
  let credentials = vcapUtils.getServiceCredentials(service => service.credentials && service.credentials.endpoints && !!service.credentials.endpoints['token-service']);
  if (credentials) {
...
async function getUaaOptions(req) {
  return new Promise((resolve, reject) => {
    uaaUtils.getUaaConfig(req, (err, uaaConfig) => {
      ...
      uaaConfig.url = uaaConfig.url.replace(/\/$/, '');
      uaaConfig.certurl = uaaConfig.certurl && uaaConfig.certurl.replace(/\/$/, '');
