...
function _addAppToResult(app, result, cachedAppElement, runtimeHost) {
  ...
  for (const url of cachedAppElement.urls) {
    ...
    if (appkey) {
      newApp.applicationName = appkey.appName;
      ...
      if (appkey.appDestinationId) {
        ...
        if (embeddedCredsApp) {
          newApp.uniqueUrl = `${runtimeHost}${app.appHostId}.${sapCloudServiceNoDots}.${appkey.appName}-${appkey.appVersion}`;
        }
      }
      result.push(newApp);
    }
...
function handleDestinations(configuration) {
  ...
  destinations.forEach((destination) =>{
    let normalizedDestination = destinationUtils.normalizeDestination(destination);
  ...
  destinationUtils.adjustDestinationProperties(normalizedDestinations);
