...
const axiosRequest = async (method, requestOptions) => {
  ...
  if (enableRetry) {
    ...
    response = await axiosInstance(options).catch(err => error = err);
  } else {
    // Make the request without retries
    response = await axios(options).catch(err => error = err);
  }
