...
module.exports.getDbId = async () => {
    LOG.info('Retrieving database ID from existing containers...')
    const hana = require('@sap/cds-mtxs/srv/plugins/hana/srv-mgr')
    const all = await hana.getAll()
    const foundDbId = all.length ? all[0].credentials?.database_id : null
    LOG.info(`Found database ID: ${foundDbId || 'none'}`)
    return foundDbId
}
...
module.exports.wasOldMtx = async () => {
    if (!wasOldMtx.length) {
        if (cds.env.requires.db?.credentials?.sm_url) { // old mtx was service-manager based
            const hana = require('@sap/cds-mtxs/srv/plugins/hana/srv-mgr')
            try {
